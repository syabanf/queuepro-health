...
function MobileServiceCard({ service, queues, participants }) {
  ...
  return (
    <div className={`rounded-xl overflow-hidden border transition-all duration-500
      ...
      {/* Header */}
      <div className={`flex items-center gap-2 px-3 py-1.5 ${isMedical ? "bg-blue-500/25" : "bg-cyan-500/25"}`}>
        <div className={`w-6 h-6 rounded flex items-center justify-center flex-shrink-0 ${isMedical ? "bg-blue-400/30" : "bg-cyan-400/30"}`}>
        ...
        <div className="flex gap-2.5 flex-shrink-0 text-right">
          <div>
            <p className="text-white font-bold text-sm leading-none">{waiting.length}</p>
            <p className="text-white/40 text-[9px]">Tunggu</p>
          </div>
          <div>
            <p className="text-green-300 font-bold text-sm leading-none">{doneCount}</p>
            <p className="text-white/40 text-[9px]">Selesai</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 divide-x divide-white/10">
        ...
        <div className="flex flex-col items-center justify-center py-2 px-2">
          <p className="text-white/40 text-[9px] uppercase tracking-wider mb-0.5">Berikutnya</p>
          <p className={`font-black tracking-widest leading-none ${next ? "text-2xl text-white/60" : "text-2xl text-white/15"}`}>
            {next ? next.queue_number : "—"}
          </p>
          {!next && <p className="text-white/25 text-[9px] mt-0.5">Kosong</p>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col" style={{
      ...
      <div className="flex-shrink-0 px-3 pb-2">
        <p className="text-center text-white/30 text-[9px] uppercase tracking-widest">{eventTagline}</p>
      </div>
    </div>
  );
}
